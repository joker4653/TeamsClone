...
def test_pin_message_auth_user_not_member_in_channel_or_dm(example_user_id, example_messages):
...
def test_pin_message_auth_user_not_owner_in_channel_or_dm(example_user_id, example_messages):
...
def test_pin_message_success(example_user_id, example_channels, example_messages):
    response1 = process_test_request(route="/message/pin/v1", method='post', inputs={
        'token': example_user_id[1].get('token'), 
        'message_id': example_messages[1].get('message_id')
    })
