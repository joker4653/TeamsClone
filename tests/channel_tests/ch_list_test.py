...
def test_list_non_member(example_user_id):
    ...
    process_test_request(route="/channels/create/v2", method='post', inputs={'token': example_user_id[0].get('token'), 'name': "cool_channel", 'is_public': False})
    response = process_test_request(route = "/channels/list/v2", method = 'get', inputs = {'token': example_user_id[0].get('token')})
