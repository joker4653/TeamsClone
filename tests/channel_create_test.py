...
def test_create_channel_public(example_user_id):
    ...
    assert data_store['channels'][0]['channel_id'] == new_channel_id
    assert data_store['channels'][0]['name'] == "I_love_seams"
    assert data_store['channels'][0]['is_public'] == True
    assert data_store['channels'][0]['user_ids'][0] == example_user_id
...
def test_create_channel_private(example_user_id):
    ...
    store = data_store.get()
    assert data_store['channels'][0]['channel_id'] == new_channel_id
    assert data_store['channels'][0]['name'] == "Badgers"
    assert data_store['channels'][0]['is_public'] == False
    assert data_store['channels'][0]['user_ids'][0] == example_user_id
